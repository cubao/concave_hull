...
template <class T>
bool intersects(const std::array<T, 3> &p1, const std::array<T, 3> &q1,
                const std::array<T, 3> &p2, const std::array<T, 3> &q2)
{
...
template <class T>
T sqSegDist(const std::array<T, 3> &p, const std::array<T, 3> &p1,
...
// segment to segment distance, ported from
// http://geomalgorithms.com/a07-_distance.html by Dan Sunday
template <class T>
...
template <class T, int DIM, int MAX_CHILDREN, class DATA> class rtree
{
  public:
        ...
        for (auto i = 0; i < DIM; i++)
            if (bounds[i] > bounds[i + DIM])
                throw std::runtime_error(
                    "Bounds minima have to be less than maxima");
    }
    ...
    void
    intersection(const bounds_type &bounds,
    ...
    std::vector<std::reference_wrapper<const_type>>
    intersection(const bounds_type &bounds) const
    {
...
//             std::cout << (i++) << std::endl;
    ...
    CircularElement<T> *insert(element_type *prev, Args &&...args)
    {
        ...
        if (prev == nullptr && m_last != nullptr)
            throw std::runtime_error(
                "Once the list is non-empty you must specify where to insert");
...
#ifdef DEBUG_2
template <class T>
void snapshot(const std::array<T, 3> &a, const std::array<T, 3> &b,
              const std::array<T, 3> &c, const std::array<T, 3> &d,
              const double sqLen, const double maxSqLen,
              const std::array<T, 3> &trigger, const bool use_trigger)
{
    ...
    if (!use_trigger || trigger == b) {
        ...
        printf("... segment c, d: %0.6f %0.6f, %0.6f %0.6f \n", c[0], c[1],
               d[0], d[1]);
        printf("... sqDist a-b, b-c, c-d: %e, %e, %e", getSqDist(a, b),
               getSqDist(b, c), getSqDist(c, d));
...
concaveman(const std::vector<std::array<T, 2>> &points,
           // start with a convex hull of the points
           const std::vector<int> &hull,
           // a relative measure of concavity; higher value means simpler hull
           T concavity = 2,
           // when a segment goes below this length threshold, it won't be
           ...
           T lengthThreshold = 0)
{
    ...
    while (!queue.empty()) {
...
#ifdef DEBUG_2
        // dump key parameters either on every pass or when a certain point is
        // 'b'
        ...
        if (ok && std::min(getSqDist(p, b), getSqDist(p, c)) <= maxSqLen) {
            ...
            // connect the edge endpoints through this point and add 2 new edges
            // to the queue
            ...
            segTree.insert(elem, {node.minX, node.minY, node.maxX, node.maxY});
            segTree.insert(elem->next(),
                           {next.minX, next.minY, next.maxX, next.maxY});
        }
#ifdef DEBUG
        else
    ...
    std::vector<std::array<T, 2>> concave;
    for (auto elem = last->next();; elem = elem->next()) {
        std::array<T, 2> p{elem->data().p[0], elem->data().p[1]};
        concave.push_back(p);
        if (elem == last)
            break;
    }
...
template <class T, int MAX_CHILDREN>
std::vector<int> concaveman_indexes(
    const std::vector<std::array<T, 2>> &points,
    // start with a convex hull of the points
    const std::vector<int> &hull,
    // a relative measure of concavity; higher value means simpler hull
    T concavity = 2,
    // when a segment goes below this length threshold, it won't be drilled down
    // further
    T lengthThreshold = 0)
{
...
#ifdef DEBUG
    std::cout << "concaveman_indexes()" << std::endl;
#endif
    ...
    // convert the resulting hull linked list to an array of points
    std::vector<int> concave_indexes;
    for (auto elem = last->next();; elem = elem->next()) {
        concave_indexes.push_back(int(elem->data().p[2]));
        if (elem == last)
            break;
    }

    return concave_indexes;
}

template <class T, int MAX_CHILDREN>
std::array<T, 3>
findCandidate(const rtree<T, 2, MAX_CHILDREN, std::array<T, 3>> &tree,
              const std::array<T, 3> &a, const std::array<T, 3> &b,
              const std::array<T, 3> &c, const std::array<T, 3> &d, T maxDist,
              const rtree<T, 2, MAX_CHILDREN,
                          typename CircularElement<Node<T>>::ptr_type> &segTree,
              bool &ok)
{
    ...
    std::priority_queue<tuple_type, std::vector<tuple_type>,
                        compare_first<tuple_type>>
        queue;
    std::reference_wrapper<const_tree_type> node = tree;

    // search through the point R-tree with a depth-first search using a
    // priority queue in the order of distance to the edge (b, c)
    while (true) {
        ...
        while (!queue.empty() && std::get<1>(queue.top()).get().is_leaf()) {
            ...
            // skip all points that are as close to adjacent edges (a,b) and
            // (c,d), and points that would introduce self-intersections when
            // connected
            ...
            if (-std::get<0>(item) < d0 && -std::get<0>(item) < d1 &&
                ...
                std::cout << "Not OK: " << cond1 << " " << cond2 << " " << cond3
                          << " " << cond4 << std::endl;
            }
#endif
        }
...
T sqSegBoxDist(const std::array<T, 3> &a, const std::array<T, 3> &b,
...
template <class T, int MAX_CHILDREN, class USER_DATA>
bool inside(const std::array<T, 3> &a,
            const rtree<T, 2, MAX_CHILDREN, USER_DATA> &bbox)
{
...
bool noIntersections(
    const std::array<T, 3> &a, const std::array<T, 3> &b,
    const rtree<T, 2, MAX_CHILDREN, typename CircularElement<Node<T>>::ptr_type>
        &segTree)
{
